...
namespace Common;
final class Image{
	public static function imageWaterMark($groundImage,$waterPos=0,$waterImage="",$waterText="kuaichuang",$textFont=5,$textColor="#FF0000",$w_alpha="") 
	{
		global $webdb;
	...
	*把图片缩小
	**/
	public static function ResizeImage($oldpic,$newpic,$maxwidth=800,$maxheight=600){
		/*if( preg_match('.jpg',$oldpic) ){
			 $im = imagecreatefromjpeg($oldpic);
		}elseif( preg_match('.png',$oldpic) ){
			 $im = imagecreatefrompng($oldpic);
		}elseif( preg_match('.gif',$oldpic) ){
			$im = imagecreatefromgif($oldpic);
		}else{
			exit(json_encode(array("status"=>0,"msg"=>"图片格式不对$oldpic")));
		}*/
		if( substr(strrchr($oldpic, '.'), 1) == "jpg" ||  substr(strrchr($oldpic, '.'), 1) == "jpeg"){
			 $im = imagecreatefromjpeg($oldpic);
		}elseif( substr(strrchr($oldpic, '.'), 1) == "png" ){
			 $im = imagecreatefrompng($oldpic);
		}elseif( substr(strrchr($oldpic, '.'), 1) == "gif" ){
			$im = imagecreatefromgif($oldpic);
		}else{
			exit(json_encode(array("status"=>0,"msg"=>"图片格式不对$oldpic")));
		}
	...
	public static function cutimg($srcimgurl,$endimgurl,$x,$y,$endimg_w,$endimg_h,$border_w,$border_h,$scale=100,$fix=0){
	...
	public static function gdfillcolor($srcFile,$dstFile,$dstW,$dstH){ 
	...
	//认证码
	public static function yzImg($nmsg){
	...
	public static function yz2img($ck){
	...
	//power by pw
	public static function PWImgWaterMark($source, $w_pos = 0, $w_img = '', $w_text = '', $w_font = 12, $w_color = '#FF0000', $w_pct, $w_quality, $dstsrc = null) {
	...
	public static function PWMakeImage($type, $image, $filename, $quality = '75') {
	...
	public static function PWGetImgSize($srcFile, $srcExt = null) {
	...
	public static function PWGetImgInfo($srcFile) {
	...
	//汉字验证码
	public static function chinese_yzimg($str){
		$fnt = SYSTEM_RESOURCES_PATH."/chinese/font.ttf";
		if(WEB_LANG=='gb2312'){
			$str = gbk2utf8($str);
		}	
		$c='red';
		...
		$y=0;
		$size=11;
		$image=SYSTEM_RESOURCES_PATH.'/chinese/chinese_yzimg.jpg';
		if(!is_file($image)){
			die('背景图片不存在');
		}
	 ...
	 *图像处理函数
	 **/
	public static function gdpic($srcFile,$dstFile,$width,$height,$type=''){
		if(is_array($type)){
			//截取一部分,以满足匹配尺寸
			self::cutimg($srcFile,$dstFile,$x=$type[x]?$type[x]:0,$y=$type[y]?$type[y]:0,$width,$height,$x2=$type[x2]?$type[x2]:0,$y2=$type[y2]?$type[y2]:0,$scale=$type[s]?$type[s]:100,$fix=$type[fix]?$type[fix]:'');
		}elseif($type==1){
			//成比例的缩放
			self::ResizeImage($srcFile,$dstFile,$width,$height);
		}else{
			//与尺寸不匹配时.用色彩填充
			self::gdfillcolor($srcFile,$dstFile,$width,$height);
		}
	}
}
?>