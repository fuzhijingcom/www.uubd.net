...
abstract class Validation{
	...
	//验证是否为合法的国际手机号码
	public static function IsMobileNumber($str){$str = html_entity_decode($str);return preg_match("/^1[0-9]{10}$/",$str) ? true : false;}

	//验证是否为合法的手机号码
    public static function checkTelephone($str){$str = html_entity_decode($str);return preg_match("/^1[3-8]{1}[0-9]{9}$/",$str) ? true : false;}
	...
	//验证是否为合法的url地址
	public static function IsUrlAdress($str){$str = html_entity_decode($str);return (preg_match("/^https?:\/\/[A-Za-z0-9-]+\.[A-Za-z0-9-]+[\/=\?%\-&_~`@[\]\':+!]*([^<>\"\"])*$/",$str))?true:false; }
	...
	//判断是否是手机或座机号码
	public static function IsTel($value){$mobile = "/^(((13[0-9]{1})|(15[0-9]{1})|(18[0-9]{1})|(17[0-9]{1}))+\d{8})$/";$tel = "/^(\d{3,4}-?)?\d{7,9}$/g"; if(preg_match($mobile, $value) || preg_match($tel, $value)){return true;} return false;}
