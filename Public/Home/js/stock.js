...
function initWareHouse(data) {
    ...
    $.each(JSON.parse(data), function (index, val) {
        _html += "<option value='" + val.w_name + "'>" + val.w_name + "</option>";
    });
    obj.html(obj.html() + _html);
    $("#location option[value='总仓']").attr("selected", true);
}

var openingGoodsPop = null;
...
function closeOpeningPop(tid, flag) {
    return function __closeOpeningPop(even) {
        ...
        if (openingGoodsPop == 'add-goods-id') {

            var targetId = even.target.id;
            if(targetId != 'add-goods-btn-id' && e.target.title != 'pop'){
                console.log('opengingGoodsPop close:');
                console.log(openingGoodsPop);
                var oTd = document.getElementById(openingGoodsPop);
                console.log(oTd);
                if (oTd) {
                    var oTr = oTd.parentNode.parentNode;
                    // console.log(oTr);
                    // oTr.className = '';
                    $(oTr).popover('hide');
                    openingGoodsPop = null;
                }
                return;
            }
...
function searchStock() {
    ...
    var ajaxData = {
        ...
        is_listing: productStockType.value,
        cat_id: cat_id,
    ...
    $.ajax({
        ...
        success: function (res) {
            if (res.sign == 1) {
                ...
                initTable(res.result.goods_list, 'refresh', pType, position.value, cat_id);
            } else {
                addNoitemInfo();
            }
        }
...
//数量加减功能
function numPlusOrMinusEdit(e, n) {
    var event = window.event || e;
    var target = event.srcElement || event.target;
    // console.log(target);
    if (target.tagName == 'A') {
    ...
    if (num != '') {
        ...
        switch (n) {
            ...
            case 0:
                ...
                if (changeQuantities(vNumBox, num, -1) === false) {
                    return;
                }
...
function checkStock(e) {
    ...
    $v = $(target);
    var num = $v.val();
    var hiddenNum = $v.nextAll("input[name='hidden-quantity']").val();
    ...
    num = parseInt(num);
    hiddenNum = parseInt(hiddenNum);
    var deltaNum = num - hiddenNum;

    if (changeQuantities(target, num, deltaNum) === false) {
        alert('数量太大或太小');
        target.value = "";
        $v.nextAll("input[name='hidden-quantity']").val(hiddenNum);
    }
...
// 加减操作各种事件操作统一处理函数
function changeQuantities(e, num, deltaNum) {
    if (changeSpecQuantity(e, num, deltaNum) === false) {
        return false;
    }

    if (changeMainQuantity(e, - deltaNum) === false) {
        changeSpecQuantity(e, num, - deltaNum);  // 需将前面动作回退
        return false;
    }
}

// 总仓数量增减操作
function changeMainQuantity(e, deltaNum) {
    $v = $(e);

    // var mainNum = $("#main-warehouse .new-quantity:first").text();
    var mainNum = $v.parent().parent().parent().siblings('.main-warehouse').eq(0).find('.new-quantity').eq(0).text();
    mainNum = parseInt(mainNum) + deltaNum;
    if (mainNum < 0) {
        return false;
    }
    // $("#main-warehouse .new-quantity:first").text(mainNum);
    $v.parent().parent().parent().siblings('.main-warehouse').eq(0).find('.new-quantity').eq(0).text(mainNum);

    // var mainDeltaNum = $("#main-warehouse input[name='delta-quantity']").val();
    var mainDeltaNum = $v.parent().parent().parent().siblings('.main-warehouse').eq(0).find("input[name='delta-quantity']").val();
    console.log(mainDeltaNum);
    mainDeltaNum = parseInt(mainDeltaNum) + deltaNum;
    // $("#main-warehouse input[name='delta-quantity']").val(mainDeltaNum);
    $v.parent().parent().parent().siblings('.main-warehouse').eq(0).find("input[name='delta-quantity']").val(mainDeltaNum);

    // var mainHiddenDeltaNum = $("#main-warehouse input[name='hidden-quantity']").val();
    var mainHiddenDeltaNum = $v.parent().parent().parent().siblings('.main-warehouse').eq(0).find("input[name='hidden-quantity']").val();
    mainHiddenDeltaNum = parseInt(mainHiddenDeltaNum) + deltaNum;
    // $("#main-warehouse input[name='hidden-quantity']").val(mainHiddenDeltaNum);
    $v.parent().parent().parent().siblings('.main-warehouse').eq(0).find("input[name='hidden-quantity']").val(mainHiddenDeltaNum);
}


// 对应非总仓的门店库存数量增减操作
function changeSpecQuantity(e, num,  deltaNum) {
    var $v = $(e);

    var newNum = $v.parent().parent().prev().children(".new-quantity").eq(0).text();
    newNum = parseInt(newNum) + deltaNum;
    if (newNum < 0) {
        return false;
    }
    $v.parent().parent().prev().children(".new-quantity").eq(0).text(newNum);

    $v.nextAll("input[name='hidden-quantity']").val(num);
}

// 判断是否为整数
function isInteger(obj) {
    return String(parseInt(obj)) === obj;
}
...
function changeStockModule(e) {
    ...
    if (target == activeEl) {
        ...
        // 判断是否为“变更记录”
        if (target.innerHTML == '变更记录') {
            $("#product-type a").removeClass("active");
            $("#product-type a:first").addClass("active");

            $("#product-type #modules-box").css("visibility","hidden");
        } else {
            $("#product-type #modules-box").css("visibility","visible");
        }
...
function renderContent(module) {
    ...
    switch (module) {
        ...
        case '库存调换':
            ...
            tableHead.innerHTML = component.tablePart.K;
            tableHead.title = 'stock-transfer';
            $("#salesDate").daterangepicker(null, function (start, end, label) {
            });
            $("#salesDate").val(initDate);
            initViewSelect();
...
function renderCategory(module) {
    ...
    switch (mod) {
        case '库存一览':
            controlBox.innerHTML = component.controlPart.searchBox;
            console.log(component.tablePart[module]);
